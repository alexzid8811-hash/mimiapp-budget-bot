...
def cashflow_snapshot(user_id: int) -> dict:
    ...
    # Do not hide a card overdraft behind a zero balance. A negative amount
    # belongs to the card, not to the buffer. It must carry into the next
    # budget period and reduce its newly calculated daily allowance. Likewise,
    # the card cannot promise more than its actual available balance, even at
    # the start of a newly calculated period.
    remaining_period = round(
        min(
            period_budget - spent_period - transferred_period,
            available_cash,
        ),
        2,
    )
